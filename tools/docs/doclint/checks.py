...
def skill_front_matter_violations(skill_name: str, meta: Dict[str, str],
    ...
    **代わりに、読み手がフロントマターとして読めない形を断る**: 閉じ `---` が無い・欄が空・
    ...
    からこそ検査で止める——読めない形を置いて「動いているつもり」になるのを防ぐ）。
...
def check_updated_freshness(docs: List[Doc], findings: List[Finding]) -> None:
    ...
    **飛ばしたことは黙らず印字する**（黙って素通りする検査を作らないため）。
...
#
# **この検査の限界**（過大に表明しない。qa/02 の R22-05「限界は検査のそばに置く」）:
...
# 発効日前は件数を印字、発効日以後は error にする（30 §9 の「成果物を自動で検査する仕組み」。ADR-0043）。
#
...
# 印は発効日以後にしか効かないので、そこに付けると当日に直すべき行が検査から消える（qa/02 の R44-01）。
#
# **この検査の限界**（過大に表明しない。qa/02 の R22-05）:
