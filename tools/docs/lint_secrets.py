...
検査対象は `git ls-files` が返す追跡ファイルの**中身**のみ（追跡外は公開されないため対象外）。

**コミットのメタデータ（著者名・メールアドレス・コミットメッセージ）は見ていない。**
**著者が実名と会社ドメインのメールアドレスであるのは故意である**——
「業務で作成しているものですから、本名と会社のメールアドレスを使っています。問題ありません」
（開発者。2026-09-08。指摘は同日の自己レビュー。docs/qa/02 のラウンド 55）。
**同じ指摘が /security-review で再発したら、ここを見せて閉じてよい。**
