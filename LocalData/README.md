...
> **`designer.settings.Development.json` に `//` コメントを書かないこと。** JSON として不正になり、デザイナは接続を確立できないまま**エラーを出さずに続行**する。結果 `designcheck` が「テーブル 'xxx' がデータソースに存在しません」を全モジュール分（実測 73 件）報告し、原因の見当がつかない。
> 注意書きを残したいときは `"_README": "..."` のような**未知のキー**にする（読み込み時に無視されることを 2026-08-18 に実測。→ `docs/15_再利用性レビュー/02_抜き取り実験.md`）。
> サーバ側の `appsettings.Development.json` は ASP.NET Core の JSON プロバイダがコメントを許容するためこの制約を受けない（雛形のコメントもそのままでよい）。
