...
public class JournalPostingRejectedExceptionTests
{
    ...
    {
        var error = new JournalPostingRejectedException(
            [.. Enumerable.Range(1, 21).Select(i => new Violation($"I-{i:D2}", $"{i} 件目。"))]);

        // **番号は 20 個すべてを固定する。** 端だけ見ていると、間の記号が化けても気づけない
        // （2026-09-24 に⑩から⑳へ広げた——理由はたいてい 2〜3 文なので、番号が切れると次の理由が前の続きに見える）。
        Assert.Equal(
            "計上できません（21 件）。①1 件目。②2 件目。③3 件目。④4 件目。⑤5 件目。"
            + "⑥6 件目。⑦7 件目。⑧8 件目。⑨9 件目。⑩10 件目。"
            + "⑪11 件目。⑫12 件目。⑬13 件目。⑭14 件目。⑮15 件目。"
            + "⑯16 件目。⑰17 件目。⑱18 件目。⑲19 件目。⑳20 件目。21 件目。",
            error.Message);
    }
