...
/// </remarks>
public class AccountingTimeZoneTests
{
    [Fact]
    public void 壁時計の時刻を日本標準時の時点として読む()
    {
        var instant = AccountingTimeZone.FromWallClock(new DateTime(2026, 8, 24, 13, 6, 46));
    ...
    public void 時点を日本標準時の壁時計に直す()
    {
        ...
        Assert.Equal(new DateTime(2026, 8, 24, 13, 6, 46), AccountingTimeZone.ToWallClock(instant));
    }
    ...
    public void 書いて読むと同じ時点に戻る()
    {
        ...
        Assert.Equal(instant, AccountingTimeZone.FromWallClock(AccountingTimeZone.ToWallClock(instant)));
    }

    [Fact]
    public void 候補は順に試す()
        => Assert.Equal(AccountingTimeZone.Value, AccountingTimeZone.Resolve(["存在しない", "Asia/Tokyo"]));

    [Fact]
    public void 見つからなければ落とす_黙って_UTC_にしない()
    {
        var error = Assert.Throws<InvalidOperationException>(
            () => AccountingTimeZone.Resolve(["存在しない"]));
