...
public static class JournalLineRules
{
    ...
    /// <summary>
    /// 変える・消す明細が消えていた。<b>件数を入れた 1 文で断る</b>——差分に行番号は無いので行は指せず、
    ...
    /// <summary>
    /// 変える・消す明細を、開いたあとに別の人が変えていた（明細の版が食い違う）。<b>件数を入れた 1 文で断る</b>——消えた明細と同じ理由。
    /// </summary>
    // 伝票の変更と同じく、入力内容は悪くない。開き直して、先に保存した人の変更を確かめるよう案内する。
    public static string LinesChangedByOthers(int count)
        => $"明細 {count} 行が、あなたが開いたあとに別の人に変更されています。画面を開き直して、その変更を確かめてから、もう一度操作してください。";

    /// <summary>
    /// 明細を変える・消す・足す伝票が、開いたあとに<b>計上されていた</b>。計上した伝票は変えられないので、直すときの手まで言う。
    /// </summary>
    // 「訂正する」は画面のボタンの字そのもの（docs/21 §2-6）。
    public const string PostedByOthers =
        "この伝票は、あなたが開いたあとに計上されました。計上した伝票は変えられないので、画面を開き直して、直すときは「訂正する」を使ってください。";

    /// <summary>
    /// 開いたあとに<b>計上された</b>伝票を削除しようとした。古い下書きの画面には取消・訂正のボタンが無いので、開き直すところから言う。
    /// </summary>
    public const string PostedByOthersOnDelete =
