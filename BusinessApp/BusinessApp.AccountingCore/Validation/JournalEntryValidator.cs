...
public static class JournalEntryValidator
{
    ...
    private static void ValidateStructure(JournalEntry entry, List<Violation> violations)
    {
        ...
        {
            violations.Add(new Violation(ViolationCodes.NoLines, "明細が 1 行もない。"));
            return;
        }
        ...
        {
            violations.Add(new Violation(ViolationCodes.DuplicateLineNo, "行番号が重複している。", lineNo));
        }

        if (entry.EntryType.RequiresOriginalEntry() && string.IsNullOrEmpty(entry.OriginalEntryId))
        {
            violations.Add(new Violation(
                ViolationCodes.OriginalEntryMissing,
    ...
    private static void ValidateLines(JournalEntry entry, IAccountLookup accounts, List<Violation> violations)
    {
        ...
        foreach (var line in entry.Lines)
        {
            ...
            {
                violations.Add(new Violation(
                    ViolationCodes.AmountNotPositive,
            ...
            {
                violations.Add(new Violation(
                    ViolationCodes.TaxCategoryMissing,
            ...
            {
                violations.Add(new Violation(
                    ViolationCodes.AccountInactive,
            ...
            {
                violations.Add(new Violation(
                    ViolationCodes.DepartmentMissing,
        ...
        List<Violation> violations)
    {
        if (!line.IsTaxLine)
        {
            ...
            {
                violations.Add(new Violation(
                    ViolationCodes.TaxLineParentInvalid,
                    "本体行に親行を指定できない。",
                    line.LineNo));
            }
            return;
