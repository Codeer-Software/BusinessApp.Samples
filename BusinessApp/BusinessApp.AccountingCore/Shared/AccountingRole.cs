...
{
    /// <summary>
    /// 伝票に対する操作をしてよい役割か（取消・訂正）。
    /// </summary>
    /// <remarks>
    /// <para><b>担当と責任者。</b> 取消・訂正は「伝票を起こす」のと同じ人の仕事で、
    /// 画面でも同じ役割に開いてある（<c>JournalEntry</c> の書き込み条件）。
    /// <b>帳簿閲覧は含めない</b>——読むための役割である。</para>
    /// <para><b>アプリケーション層は、計上済みを動かさない操作もこの役割で守ることがある</b>
    /// （ADR-0048 の決定 9）。ただし<b>理由が違う</b>——そちらは「起票できるか」で決まる。
    /// <b>集合が一致しているのはいまだけ</b>なので、役割を増やす日はそちらを分ける
