...
/// <para>関門はどちらも保存の前に検査するだけなので、<b>通すかどうか</b>に順番の意味は無い。
/// <b>断りの出方には意味がある</b>——2 つの関門は束ねない（docs/21 §2-6 の例外）ので、外側（登録番号の関門）の断りが先に出て、
/// 内側（取引先の関門）の断りは 2 回目の保存で出る（2 つが同じ保存に来るのは API だけ）。
/// どちらが例外を投げても、呼び出し側のトランザクションごと巻き戻る。</para>
/// <para><b>取引先だけを載せるときに足りなくなるものが 3 つある。</b>
/// ①保存が失敗したときの文言を利用者の語に差し替える網（会計側の <c>SaveFailureMessage</c>）、
/// ②空にした文字の欄を NULL へ寄せる正規化（<c>BlankTextNormalizer</c>）、
/// ③<b>使用中の取引先のコードを凍結する関門</b>（会計側の <c>MasterMeaningGate</c> が取引先の行も見る。
/// ADR-0047 の決定 9。<b>「使用中」は計上済みの仕訳が参照しているかという会計の事実</b>なので、
/// 判定ごと会計コアに置いた——**取引先だけのホストには仕訳が無いので、凍結する対象も無い**）。
/// いずれも会計コアの入口に置いてある（ADR-0025 §2「2 つ以上の部品が実際に使うものだけを
/// 共有へ出す」に従い、先回りして写しを作っていない）。
/// <b>取引先だけのホストを作るときは、①と②を <c>BusinessApp.ServerSupport</c> へ移してここから呼ぶ。</b>
/// <b>③は移さなくてよい</b>（守る相手がそのホストに存在しない）。
