...
public sealed class PartnerRegistrationSubmitGate(PartnerRegistrationStore store)
{
        ...
        foreach (var (a, b) in Pairs(ordered))
        {
            ...
            if (b.From < aEnd)
            {
                throw new PartnerRegistrationRejectedException(
                    ...
                    + "始まっています。「登録年月日」と「取消・失効年月日」を確かめてください。");
            }
        }
    }
    ...
    /// 登録年月日の打ち間違いなのに<b>いま有効な登録に架空の終わりを付けて通してしまう</b>——
    ...
    /// <item><b>新規・その他の更新</b>——あとの登録までに終わった登録なら終わりを入れ、そうでなければ登録年月日の誤り</item>
    /// <item><b>保存済みの行</b>（あとの行を入力した）——前の登録が終わっているなら先にそちらを閉じ、そうでなければ入力した登録年月日の誤り（REG-23 の再登録の形）</item>
    /// </list>
    /// </remarks>
    private static string OpenRowBefore(DateOnly openFrom, RowOrigin openRow, DateOnly laterFrom)
        => openRow switch
        {
            ...
            RowOrigin.Stored =>
                ...
                + $"前の登録が {laterFrom:yyyy/MM/dd} までに終わっているなら、先に取引先の詳細の「登録番号の履歴」でその行の「編集」を開き、"
                + "「取消・失効年月日」を入れて「取消・失効の理由」を選んでください。"
                + "そうでなければ、入力した「登録年月日」を確かめてください。",
            _ =>
                "登録の期間が重なっています。"
                + $"入力している登録（{openFrom:yyyy/MM/dd} から）は「取消・失効年月日」が空ですが、"
                + $"そのあとに {laterFrom:yyyy/MM/dd} からの登録があります。"
                + $"{laterFrom:yyyy/MM/dd} までに終わった登録なら「取消・失効年月日」（{laterFrom:yyyy/MM/dd} 以前）を入れて「取消・失効の理由」を選び、"
