...
/// <para><b>順番は仕訳が外・登録が内。</b> 仕訳の関門は保存の前後でやることがある
/// （入力年月日を打つ・保存後に読み直して計上する）ので、保存そのものを包む必要がある。
/// 登録の関門は保存の前に検査するだけなので、内側でよい。
/// どちらが例外を投げても保存ごと巻き戻る。</para>
/// </remarks>
public sealed class AccountingSubmitPipeline(JournalSubmitGate journals, PartnerRegistrationSubmitGate registrations)
{
        ...
        Func<Task<List<ModuleSubmitResult>>> save)
    {
        ...
        return journals.SubmitAsync(
            transactionData,
            () => registrations.SubmitAsync(transactionData, save));
    }
}
