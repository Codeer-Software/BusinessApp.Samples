...
/// （<c>JournalPoster</c> が順番を持っている）。</para>
/// </remarks>
public sealed class LedgerSnapshotWriter(IDbAccessor dbAccessor, string dataSourceName, PartnerRegistrationStore partners)
{
    ...
    public async Task<IReadOnlyList<Violation>> BurnAsync(JournalEntry draft)
    {
        ...
        var entryName = draft.PartnerId is PartnerId entryPartnerId
        ...
        foreach (var line in draft.Lines)
        {
            ...
            resolved.Add((line.LineNo, snapshot.Name, registrationNo));
        }
        ...
        await WriteEntryNameAsync(id, entryName);
        foreach (var (lineNo, name, registrationNo) in resolved)
        {
            await WriteAsync(id, lineNo, name, registrationNo);
        }
    ...
    /// <remarks>
    /// <b>どれを写すか決められないときは、利用者のことばにする。</b>
    /// 純粋関数（<see cref="InvoiceRegistrationHistory"/>）が投げるのは開発者向けの文言なので、
    /// ここで利用者向けの断りに載せ替える（docs/21_画面の原則.md §2）。差し戻しの例外にするのは呼び手の仕事。
    /// </remarks>
    private static (string? RegistrationNo, Violation? Violation) RegistrationNoAt(PartnerSnapshot snapshot, DateOnly taxPoint)
    {
        try
        {
            return (InvoiceRegistrationHistory.InEffectOn(snapshot.Registrations, taxPoint)?.RegistrationNo, null);
        }
        catch (InvalidOperationException)
        {
            return (null, new Violation(
