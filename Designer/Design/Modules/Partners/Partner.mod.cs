...
// 登録番号の編集は別画面で行う（docs/14 §4。2026-09-02 の作り直し）。
...
void Code_OnSearchDataChanged()
{
    var trimmed = Code.SearchValue?.Trim();
    // 同じ字なら書き戻さない——書き戻しがこの手をもう一度呼ぶ形にしない。
    if (trimmed == Code.SearchValue) return;

    Code.SearchValue = trimmed;
}

void Name_OnSearchDataChanged()
{
    var trimmed = Name.SearchValue?.Trim();
    // 同じ字なら書き戻さない——書き戻しがこの手をもう一度呼ぶ形にしない。
    if (trimmed == Name.SearchValue) return;
