...
void ApplyPostedLock()
{
    ...
    // **入力のための字は、計上済みの画面に出さない。** 計上済みは書き込み条件から外れていて
    // 画面全体が読むだけになる（qa/01 F-14）ので、「必須項目です」も入力の決まりも、
    // これから入力する人にしか意味が無い（docs/21 §1「書けない状態の画面は、書けないと見て分かる」）。
    // **印そのものは消せない**——ClassName はレイアウトに焼かれていて、状態で切り替えられない。
    //
    // **凡例は `*` を打ち消すところまで言う**（docs/21 §1「打ち消していない印を残さない」・
    // Designer/Project.md の 2026-09-08）——**摘要と伝票の取引先は、印が出ていても常に必須ではない。**
    // **打ち消しを最下段へ移すと、印の隣に残るのは嘘になる**（2026-09-23 の自己レビューで踏んだ）。
    //
    // **入場券等の回収特例の 1 文は、明細の直前に置く**——**文言の正典は docs/13 §1-4**
    // （住所の写しを仕訳に作らないと決めた、その代わりの運用）。ここと 13 に同じ日本語があるので、
    // 直すときは両方を見る。**欄（明細の「内容」）を名指しして打鍵を指示する字なので、欄から離さない**
    // （docs/21 §2-8 の例外）。
    RequiredLegendLabel.IsVisible = !posted;
    InvoiceNoticeLabel.IsVisible = !posted;
    EntryNotesLabel.IsVisible = !posted;

    // **取消と訂正の案内は、計上済みでも出す。** **「取り消す」「訂正する」が出るのは計上済みの画面だけ**なので、
    // **未計上に限ると、押す人が 1 度も読めない**（確認ダイアログから外した 4 文がここにある。
    // 開発者の決定。2026-09-23。docs/10 §5）。
    // **注記ではなくコードで言う**——ここが `ApplyPostedLock` の「毎回入れ直す」から 1 欄だけ外れていると、
    // **誰かが 1 か所で false にした日に、戻す場所が無いまま固着する**（下の ShowAmendmentNotice で実際に起きていた形）。
    AmendGuideLabel.IsVisible = true;
...
{
    // **先に空へ戻す。** 下は 3 つの分岐の中でしか代入せず、最後に `Text` の空かどうかで `IsVisible` を決める
    // ——**戻さないと、前に開いた伝票の断りが残ったまま真になる**（この画面は使い回される。上の注記）。
    // **2026-09-23 の実機では再現しなかった**（取消済み → 未取消の順で開いても残らなかった）が、
    // **CLB が欄を作り直すかどうかに頼らない**——`ApplyPostedLock` が他の全欄でしている「毎回入れ直す」に揃える。
...
//
// **「取消とは何か」はここに書かない**（開発者の決定。2026-09-23。docs/10 §5）。
// **どの伝票でも同じ 4 文は、画面の最下段の AmendmentNotesLabel が常に出している**
// ——ダイアログに残すのは**この伝票だけの断り**と「よろしいですか？」である。
// 4 文の字の正典は docs/10 §5（開発者の承認。2026-09-20。ADR-0066 の決定 10・15）で、
// 取消の範囲は「記帳の誤り」と「遡及効のある無効・取消し」の 2 つ。
// **法令の語を 1 つも使わずに軸だけを言う**——遡及効・無効・取消し・法定解除・合意解除は画面に出さない。
void ReverseButton_OnClick()
{
    Amend("reverse", "取消",
        "この伝票を取り消します。" + PriorPeriodNotice(false) + "よろしいですか？",
...
//
// **取消も訂正も、反対仕訳に原仕訳の基準日を写す**ので、動くのは原仕訳の課税期間の税額である
// （docs/11 §5-2。ADR-0066 の決定 10 の③・決定 16）。**押す前に見せる**（docs/21 §1）。
//
// **言うのは「影響する」までで、次の一手は書かない**（開発者の決定。2026-09-23。docs/10 §5）。
// 逐語は「影響があることを伝え、このまま実行していいかどうかを聞く程度でいい」
// 「『確かめてください』すら不要」「誰に確認を求めるかは会社によって違うので当然書けない」。
// **税目も「消費税等」までにする**（同）——法人税・地方税も動くが、
// **厳密さを求め続けると分岐がいくらあっても足りない**（開発者）。
//
// **resumed は「取消が既に計上済みか」**（ADR-0052 のやり直しの訂正）。**そこだけは文を分ける**
// ——**押して起きるのは下書きが 1 本開くことだけで、税額はこの操作では 1 円も動かない**。
// 動かしたのは既に計上されている取消のほうなので、**未来形で言うと「実行していいか」を聞く材料が偽になる**。
// **開発者が「分岐を増やすな」と述べたのは税目と次の一手についてで、この分岐は問いに含まれていない**
// （docs/05 で諮っている。**これは Claude の当てはめである**）。
//
// **1 文を 2 つに割る。** 「取引日は…にあるので、その年度の…に影響します」と続けると、
// **主題の「取引日は」が後半まで係って「取引日が影響する」と読める**（2026-09-23 の自己レビュー）。
//
...
string PriorPeriodNotice(bool resumed)
{
    ...
    // **代入と分岐に割る。**
    //
    // **1 文を途中で継がない。** 受けの語だけを変数にすると、**台本と突き合わせる検体**
    // （`台本の期待値は画面の組む文と一致する`）**が、文を 2 つのリテラルにまたいで探すことになる**
    // ——隣り合っていない字は見つからないので、**ずれても赤くならない**。
    // **だから 1 文まるごとを 1 つのリテラルにする**（変数に入れるのは文であって、文の部品ではない）。
    var head = $"取引日（{earlierBasisDate}）は、いまの会計年度より前です。";
    var tail = "その日を含む年度の消費税等に影響します。";
    if (resumed)
    {
        tail = "その日を含む年度の消費税等には、取消の分が既に影響しています。";
    }

    if (earlierFiscalYearLabel != "")
    {
        head = $"取引日（{earlierBasisDate}）は「{earlierFiscalYearLabel}」にあります。";
        tail = "その年度の消費税等に影響します。";
        if (resumed)
        {
            tail = "その年度の消費税等には、取消の分が既に影響しています。";
        }
    }
...
void Description_OnSearchDataChanged()
{
    var trimmed = Description.SearchValue?.Trim();
    // 同じ字なら書き戻さない——書き戻しがこの手をもう一度呼ぶ形にしない。
    if (trimmed == Description.SearchValue) return;

    Description.SearchValue = trimmed;
}
