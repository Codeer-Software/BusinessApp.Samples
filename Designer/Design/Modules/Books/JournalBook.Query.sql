...
WHERE e.status = 'posted'
  ...
  AND (@p_partner_id IS NULL OR @p_partner_id = ''
       OR l.partner_id = @p_partner_id OR e.partner_id = @p_partner_id)
  AND (@p_keyword IS NULL OR @p_keyword = ''
       OR e.description LIKE '%' || @p_keyword || '%'
       OR l.item_description LIKE '%' || @p_keyword || '%')
  ...
  AND (@p_blank_field IS NULL OR @p_blank_field = ''
       OR (@p_blank_field = 'partner'
           AND l.partner_id IS NULL AND e.partner_id IS NULL
...
ORDER BY e.transaction_date, e.entry_no, l.line_no
