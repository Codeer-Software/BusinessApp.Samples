...
| 003 | [`003_tax_categories.sql`](003_tax_categories.sql) | 税区分 8 件（旧税率 8% の 2 件は 2026-09-24 に外した——docs/11 §1-1-1） |
...
| 006 | [`006_tax_rates.sql`](006_tax_rates.sql) | **税率 2 行**（標準・軽減。制度ルール。005 と同じ性質——下の節） |

各マスタの扱い（所有・誰が編集するか・版と削除）は [docs/12_マスタ台帳](../../docs/12_マスタ台帳.md) が持つ。
**税区分を指すときは id ではなく code で引く**（SQL・テストのデータ・実機の台本を書くとき）——seed から作った DB と開発機の稼働 DB で id が違う（稼働 DB は 0045 が消した 4・7 が欠けている）。

### 005・006 は「利用者が直さない行」である

**001〜004 は「利用者が後から直すマスタの初期値」**で（例外として、2026-09-24 にベンダーが 003 の 2 行を消した——docs/12 §1）、
**流したあとに利用者が足しても消しても、それが正しい状態**である。
**005 の 4 行と 006 の 2 行はベンダーが配り、利用者は 1 文字も直さない**（[ADR-0020](../../docs/decisions/0020-スキーマは現在形の正典で持ち変更は差分で配る.md)・[ADR-0069](../../docs/decisions/0069-制度ルールは値の種類ごとに表を分け有効期間と法源を行が持つ.md)）。
...
**既にある DB へは seed ではなくマイグレーションで届く**（`migrations/0040`・`0043`。旧税率の行を消したのは `0045`）。
