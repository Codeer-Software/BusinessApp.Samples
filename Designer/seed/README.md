...
- 2026-08-28 **デモユーザー（経理担当・経理責任者）の初期パスワードをどう持たせるか。**
  役割ごとの画面の分離を実機で見せるために足す（[ADR-0026](../../docs/decisions/0026-画面は役割で分け玄関のページフレームを置く.md)）が、
  **このリポジトリは公開する**——[CLAUDE.md](../../CLAUDE.md) §5 は「秘密（…パスワード）を書かない」と定めている。
  固定値を初期データに書いてよいか、書くならどう扱うかを決める。**フェーズ 2.5 の B の着手前**
