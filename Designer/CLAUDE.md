...
- **ブラウザでの実機確認は、確認を取らずに積極的に行う**（開発者の指示。2026-08-23）。サーバ URL とブラウザは `LocalEnvironment.md` に記録してある。依存の導入（Playwright 等）や `.claude` の許可追加だけは、勝手に広げずユーザーに諮る（**自動拒否を緩める決定は開発者**。正典は `../docs/34_開発者とClaudeの分担.md` の §1 の 8 と §1-3）
