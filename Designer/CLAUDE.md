...
- **ブラウザでの実機確認は、確認を取らずに積極的に行う**（開発者の指示。2026-08-23）。サーバ URL とブラウザは `LocalEnvironment.md` に記録してある。依存の導入（Playwright 等）や `.claude` の許可追加だけは、勝手に広げずユーザーに諮る（**機械的関門を緩める決定は開発者**。正典は `../docs/34_開発者とClaudeの分担.md` の §1 の 8 と §1-3）
...
### 4. 動作確認（ブラウザ）は積極的に行う
- **`designcheck` の緑は「読み込める」までの保証でしかない。** 計算・状態遷移・見た目は実機で見る。
  会計ロジックの不具合の絶無を期す本プロジェクトでは、実機確認を省く方がコストが高い（`../docs/31_検証のルール.md` §2）
- 画面を作った・変えた・スクリプトを足したときは、原則として実機で確認する
- **デプロイ（`tools/clb/deploy.ps1` による zip 配置）・サーバ再起動・ブラウザ操作は、
  開発者に確認を取らずに行ってよい**（開発者の指示。2026-08-23）。
  `CLB 実装 → deploy → サーバ再起動 → ブラウザで操作` を 1 つの検証ループとして回す
