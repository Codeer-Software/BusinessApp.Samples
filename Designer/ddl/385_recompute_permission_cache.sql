-- 385_recompute_permission_cache.sql — 組織事実キャッシュの一括再計算（何度でも再実行可、BusinessAppSQLite）
-- ADR-0043 以降、has_sales_access / has_accounting_access は「直接付与の真実」（ユーザー管理画面で編集）と
-- なったため再計算の対象外。導出キャッシュは is_approver / is_director の 2 本:
--   is_approver = どこかの部門に課長/部長行がある ∨ 承認テンプレートの個人指名承認者である
--   is_director = どこかの部門に部長行がある（ADR-0046: 案件編集の書込条件が参照）
-- （正典は 400/450 のトリガー。トリガーを迂回する経路の後や整合が疑わしいときにこれを流す）
