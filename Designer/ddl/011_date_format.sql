...
--
-- **`DATE` で宣言した列は 1 つ残らず当てる**（数は書かない——足すたびに腐る。
-- 数えているのは `DateFormatGuardTests` の「DATE で宣言した列は 1 つ残らず見張られている」である）。
--
-- **ここにまとめるのは、011 の時点で既にあった 5 表だけである。**
-- **011 より後に生まれた表は、その表のファイルの末尾に同じ字で置く**（例: 014）——
-- 011 の時点で存在しない表のトリガはここに書けないし、
-- **既存 DB への配達は常に末尾へ作る**ので、そちらに置けば正典と配達で作られた順が一致する。
