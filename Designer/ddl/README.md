...
| **名寄せの親は自分自身でない**（[ADR-0028](../../docs/decisions/0028-名寄せの親は同一人格の表明であり深さ1に固定する.md)） | `CHECK (parent_partner_id IS NULL OR parent_partner_id <> id)`。**断りが日本語でない**——`CHECK constraint failed: …` がそのまま出る（画面からは関門が先に鳴るので踏めない。2026-09-20 に実測）。**実測したのはこの 1 件で、他の `CHECK` も同じはずだが確かめていない** | `PartnerSubmitGate` |
