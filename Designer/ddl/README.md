...
| 取引先を要する科目の明細には取引先がある（[10 §6-2](../../docs/10_会計ドメイン設計.md)） | `BEFORE UPDATE` のトリガ（**下書き → 計上のときだけ**鳴る。**実効値**——明細が空なら伝票の取引先を見る。**「無い」は NULL だけでなく「マスタに実在しない」まで**——外部キーを切った経路が守る相手だから。**外すのは計上済みの原仕訳を写した取消だけ**——補助科目の 2 値と同じ形） | `E-PARTNER-REQUIRED`（**実在と有効は 2026-09-10 から関門も見る**（[10 §6-3](../../docs/10_会計ドメイン設計.md)）） |
