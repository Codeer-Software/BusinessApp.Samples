...
| 015 | [`015_tax_rates.sql`](015_tax_rates.sql) | **税率**（制度ルール。行までベンダーが配る。[docs/11 §1-1](../../docs/11_消費税設計.md)）。**014 と鍵も終わり方も値の形も違う**——**税率区分＋日付で引き**、**終期は NULL を許し**（条文が期限を書いていない）、**国税は万分率の整数・地方は分数**で持つ |
| 016 | [`016_vendor_row_no_replace.sql`](016_vendor_row_no_replace.sql) | **ベンダーが配る行を `REPLACE` の暗黙の DELETE から守る**（014・015 の 2 表）。**1 ファイルにまとめてあるのは 008・011 と同じくトリガの作られる順のため**——014 は既に配ってあるので、あちらの末尾に足すと正典と配達で順が食い違う |
...
| 補助科目は 2 値（[15 §1](../../docs/15_記帳の枠組み.md)・[ADR-0038 §3](../../docs/decisions/0038-使用中のマスタは意味を変えられない.md)） | `BEFORE UPDATE` のトリガ（**下書き → 計上のときだけ**鳴る。**外すのは計上済みの原仕訳を写した取消だけ**——**関門より外す範囲が小さい**。理由は 005_journals.sql の注記） | `E-SUBACCOUNT-REQUIRED` ＋ `E-SUBACCOUNT-NOT-ALLOWED` |
| 取引先を要する科目の明細には取引先がある（[15 §1-2](../../docs/15_記帳の枠組み.md)） | `BEFORE UPDATE` のトリガ（**下書き → 計上のときだけ**鳴る。**実効値**——明細が空なら伝票の取引先を見る。**「無い」は NULL だけでなく「マスタに実在しない」まで**——外部キーを切った経路が守る相手だから。**外すのは計上済みの原仕訳を写した取消だけ**——補助科目の 2 値と同じ形） | `E-PARTNER-REQUIRED`（**実在と有効は 2026-09-10 から関門も見る**（[15 §1-3](../../docs/15_記帳の枠組み.md)）） |
| 補助科目は 2 値——**マスタの側**（`uses_sub_account = 0` の勘定科目の下に補助科目を置けない。**マスタにも当てるのは Claude の読み**——[docs/05](../../docs/05_開発者への問い.md) の Q-67。[ADR-0038 §3](../../docs/decisions/0038-使用中のマスタは意味を変えられない.md)・[docs/12 §2](../../docs/12_マスタ台帳.md)） | **無い。関門だけが見ている**——CLB を通さない書き込み（CLI・SQL の直打ち）からは素通りする（帳簿は「補助科目は 2 値」の行の計上のトリガが守る。置くかは [docs/12](../../docs/12_マスタ台帳.md) の保留リスト） | `MasterSubmitGate`（補助科目の追加・移動と、勘定科目をオンからオフにする保存） |
| 補助科目が明細の勘定科目に属する（[15 §1](../../docs/15_記帳の枠組み.md)） | **無い。関門だけが見ている**——取込・CLI・SQL の直打ちからは素通りする（塞ぐのは取込と投入 API を作る回。[04 §3](../../docs/04_実装計画と現在地.md) のフェーズ 6） | `E-SUBACCOUNT-MISMATCH` |
...
| **個人事業者と法人系は互いに名寄せの親にできない**（[docs/13 §2-5](../../docs/13_取引先設計.md)） | **無い。関門だけが見ている**——取込・CLI・SQL の直打ちからは素通りする（**2026-09-20 に `sql` CLI で実証**。`entity_type` を見る DDL は値の範囲と「個人事業者に法人番号は無い」の 2 つだけ）。**塞ぐのは取込と同じ回**（[04 §3](../../docs/04_実装計画と現在地.md) のフェーズ 6） | `PartnerSubmitGate` |
...
C# のモジュール依存（[ADR-0050 §8](../../docs/decisions/0050-会計コアの層はドメイン・アプリケーション・インフラ・プレゼンテーションの4つで読み機能フォルダの下で分ける.md)）と
...
（取引先の登録テーブルまわりの未確認は [14_適格請求書発行事業者の登録](../../docs/14_適格請求書発行事業者の登録.md) の保留リストが持つ）

- 2026-09-14 **同時実行の検体が 1 本も無い**。`ux_journal_entries_single_reversal` などの部分一意索引は
  「**同時に 2 人が取り消すと、両方が『まだ取り消されていない』を読む**」ために置いてあるのに、
