...
結果 JSON を標準出力に返す。**一時ファイルを作らない**（[CLAUDE.md](../../CLAUDE.md) §3-2-2）。
...
| # | ファイル | 内容 | 層（[ADR-0019](../../docs/decisions/0019-マスタは会計が生んだ概念かで分ける.md)） |
|---|---|---|---|
...
（規則 5 ⑤一ロ）を担保する一連番号なので、後から振り直すと出力済みの帳簿との対応が切れる。
欠番があること自体は問題にならない。
...
ユーザーは会計コアの責務ではなく別部品のものであり（[ADR-0019](../../docs/decisions/0019-マスタは会計が生んだ概念かで分ける.md)）、
...
- マスタは削除ではなく**無効化**する（[ADR-0019](../../docs/decisions/0019-マスタは会計が生んだ概念かで分ける.md) が引き継ぐ三層の規則）。
  `is_active` は「入力時の候補に出すか」の意味であり、過去データの表示・検索を妨げない

## 保留リスト

- ~~2026-08-23 取引先の登録状況の別テーブル~~ → **追加した**（2026-08-25。`006_partner_registrations.sql`。
  設計は [07_取引先設計 §3](../../docs/07_取引先設計.md) で確定。ただし**取込前に読む未確認が残る**——
  07 の保留リストを参照）
