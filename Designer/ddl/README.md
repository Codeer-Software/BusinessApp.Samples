...
| 補助科目は 2 値（[10 §6](../../docs/10_会計ドメイン設計.md)・[ADR-0038 §3](../../docs/decisions/0038-使用中のマスタは意味を変えられない.md)） | `BEFORE UPDATE` のトリガ（**下書き → 計上のときだけ**鳴る。**「持てない」側は取消・訂正で見ない**——関門と同じ広さ） | `E-SUBACCOUNT-REQUIRED` ＋ `E-SUBACCOUNT-NOT-ALLOWED` |
