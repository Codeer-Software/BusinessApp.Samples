-- 0043 税率の表を作り、3 行を配る（Designer/ddl/015_tax_rates.sql・Designer/seed/006_tax_rates.sql）
--
-- **表と行を 1 本にまとめてある。** 0039／0040 を分けたのは、行の同値の網がまだ無かったからで
-- （migrations/README・ADR-0020 の帰結）、**網は 2026-09-23 に入った**（VendorRows）。
-- **だから、この回からは表と行を一緒に配ってよい。**
--
-- 正典の説明は ddl/015 と seed/006 が持つ。ここは配達物なので、同じ説明を写さない。
...
CREATE TABLE tax_rates (
    ...
    rate_kind                   TEXT NOT NULL CHECK (rate_kind IN ('standard', 'reduced', 'legacy_8')),

    -- 有効期間。**両端を含む**（ADR-0069 の決定 2。014 と同じ）。
    --
    -- **終期は NULL を許す。ここが 014 と違う。** 消税法 29 も地方税法 72 の 83 も期限を書いていない
    -- （税率リサーチ §1）。**ただし legacy_8 の終期なしは Claude の当てはめである**（Q-54）。
    ...
    -- **合計税率（10%・8%）は持たない。導く**（docs/11 §1-1。TaxRateTests が 3 区分とも検算する）。
    ...
    -- 版の字にも入れないと 3 区分の版が同じ字になる**（014 は日付だけで引くので種類と日付で足りた）。
    --
    ...
    -- **開発者が原文を示して確認した日も含む**）。
    --
    -- **有効期間と法源の版が一致しない行がありうる。** legacy_8 の行がそれで、
    -- **期間は 2019-10-01 から（当てはめ）、法源は令和元年7月1日施行版（条文を読んだ版）**である
    -- ——**条文がその期間に 6.3% を定めているという意味ではない**（seed/006 の注記と Q-54）。
...
-- **区分を鍵に含める**——含めないと、3 区分が同じ日から始められない。
--
