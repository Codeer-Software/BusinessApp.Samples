...
- **適用済みのファイルは編集しない・改名しない。** チェックサム（SHA-256・改行 LF 正規化）で
  ...
  **「コメントだけ」の条件は据え置きである。** 2026-09-17 に、トリガの `RAISE` の文言だけが違う 0038 も記録の訂正で揃えたが、
  それは**開発者の個別の決定**（逐語は「チェックサムを書き直せばいいと思いますが、あなたのほうで作業してください」）で、条件を広げたものではない。
  **理由は Claude の読み**——`-Verify` が稼働 DB と正典の同値を確かめており、直すべきは記録の側だから。
  **差分の種類は `git diff` では確かめられなかった**（0038 のコミットは編集後の 1 つだけ）ので、**退避の DB の `sqlite_master` とトリガ本文を比べた**
  （DB に載らない差——コメント・`DROP` 文——はこの方法では見えない。qa/04 の 2026-09-17）。
  **条件を広げるか、Claude が確認なしに行える条件をどう書くか、`-Verify` にチェックサム検査を足すかは [05 の Q-35](../../docs/05_開発者への問い.md) で諮っている**
