...
- 2026-08-24: **LinkField の候補ダイアログは、一覧画面とは別に絞り込みと並び順を持つ。**
  ...
  > **2026-08-30 追記——上は「こうする」であって「こうなっている」ではない。**
  > **実装が追いついていない範囲は [05 のフェーズ 2.5 の B](../docs/05_実装計画と現在地.md) が持つ**
  > （いちばん打鍵する振替伝票の入力画面で、上に書いた症状がそのまま起きている）。
  > **絞らないと決めている例外は 1 本——取引先の「名寄せの親」**である。
  > 名寄せは「同一人格の表明」であって入力候補ではなく、無効にした取引先を親にする場面が正常に起こる
  > （[ADR-0028](../docs/decisions/0028-名寄せの親は同一人格の表明であり深さ1に固定する.md)）。
  > 登録番号の取引先を絞ったままにした理由は `PartnerInvoiceRegistration.mod.cs` の冒頭にある
