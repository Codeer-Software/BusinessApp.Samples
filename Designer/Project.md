...
- 2026-07-06: **Update タイミングの ExecuteSqlField は「送信差分ゼロの Submit」では発火しない可能性**（翌期繰越ボタンで実測。非バインドフィールドだけ変えても UPDATE 文が出ない）。NextYearId を実列（fiscal_years.next_year_id）にバインド化したが、修正後の発火はブラウザ実測で未確定 → **要追加調査（バックログ）。回避策: 繰越 SQL の手動実行（sql CLI）は完全動作**（temporary/carry_exec 相当）
