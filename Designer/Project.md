...
- 2026-07-21: **ListField の自動ロード完了時の OnDataChanged はレンダリング通知を伴わない**（ハンドラ内でラベル Text を設定しても初回描画に乗らず、次のユーザー操作まで空に見える。スクリプトから this.NotifyStateChanged() を呼んでも変わらず——実測）。**初期表示に使うサマリは OnAfterInitialization で ListField.Reload() を明示的に呼んで初期化中に確定させる**（BankPosting のサマリで確立。ロードは2回走るが小さいテーブルなら許容）
