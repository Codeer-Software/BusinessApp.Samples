...
- 2026-08-24: **LinkField の候補ダイアログは、一覧画面とは別に絞り込みと並び順を持つ。**
  ...
  > | 絞らないもの | 理由 |
  > |---|---|
  > | **検索の欄**（帳簿も、振替伝票・登録番号の一覧も。会計年度・科目・補助科目・部門・取引先） | **無効にしたマスタで過去を引けなくなる**（[qa/01 X-05](../docs/qa/01_CLB静かな失敗.md)）。検索は「これから入力するもの」ではない。**並び順は付ける**——無指定だと降順で、勘定科目 105 件が逆順に並ぶ（2026-08-31 に付けた）。**2026-08-31 に「帳簿の」を外した**——振替伝票・登録番号の一覧は帳簿ではない（ADR-0027）が、検索の欄である点は同じで、狭い書き方だと新しい一覧がどの行にも当たらなくなる |
  ...
  > 親 FK を `IdFieldDesign` にしたので、絞る対象そのものが無くなった（[docs/14 §4](../docs/14_適格請求書発行事業者の登録.md)）。
...
- 2026-09-08: **選べない相手を候補に出さない絞りは、`FieldValueMatchCondition` を 1 つ足すだけで書ける**
  ...
  **一覧の列に足すときは、既にある同型の列を複製する**（`IsContra` を写して `UsesSubAccount` にした——
  `TrueText: "○"` / `FalseText: "—"` の設定はフィールド側にあるので、列の側は `FieldName` と `Label` だけでよい）。
  **どちらも designcheck は緑のままなので、実機で見る**（[docs/31 §2](../docs/31_検証のルール.md)）。
- 2026-09-08: **計上でだけ必須の欄は、`IsRequired` を立てずにラベルの印だけ出す**（伝票の `Description`。
  Claude の判断）。**この日から `*` の意味は 2 通りになった**（「保存に要る」と「計上に要る」）——
  立てると CLB の入力検査が**下書き保存まで止める**が、
  **印は最初から出す**のが原則である（[docs/10 §4-2-1](../docs/10_会計ドメイン設計.md)・[docs/21 §1](../docs/21_画面の原則.md)）。
  **`*` の意味が画面の中で 2 通りになる**ので、**凡例でその欄を名指しし**、
  `lint_design.py` の `MARK_WITHOUT_REQUIRED` に**理由つきで載せる**
  （逆向きの検査——印が出ているのに `IsRequired` でない欄——が、載っていなければ赤くなる）。
