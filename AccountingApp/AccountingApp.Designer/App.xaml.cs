using AccountingApp.Client.Shared.AITextAnalyzer;
using AccountingApp.Client.Shared.ScriptObjects;
using AccountingApp.Designer.Lib;
using AccountingApp.Designer.Lib.AI;
using AccountingApp.Designer.Lib.DbTableToModule;
using AccountingApp.Designer.Lib.ModuleToClass;
using AccountingApp.Designer.Lib.SeleniumPageObject;
...
namespace AccountingApp.Designer
{
    public partial class App : DesignerApp
    {
        protected override void OnStartup(StartupEventArgs e)
        {
            ...
            ScriptRuntimeTypeManager.AddType(typeof(AccountingApp.Client.Shared.ScriptObjects.Excel));
            ...
            BlazorRuntime.InstallBundleCss("AccountingApp.Client.Shared");

            IconCandidate.Icons.AddRange(AccountingApp.Designer.Properties.Resources.bootstrap_icons
            ...
            MainWindow.Title = "AccountingApp";
            ...
            DesignerEnvironment.AddSolutionExplorerMenu(CreateFieldClass, SolutionExplorerMenuTarget.Module, "Create Field Class");
            ...
            {
                // 全 AI チャットは DesignerEnvironment を直接持たず、テスト可能な IDesignerChatHost(本番アダプタ)経由にする。
                var chatHost = new DesignerEnvironmentChatHost(DesignerEnvironment);
                DesignerEnvironment.CreateQueryChat = editor => new QueryChat(chatHost, AISettings.Instance, editor);
                DesignerEnvironment.CreateExecuteSqlChat = editor => new ExecuteSqlChat(chatHost, AISettings.Instance, editor);
                DesignerEnvironment.CreateOverallSettingsChat = editor => new OverallSettingsChat(chatHost, AISettings.Instance, editor);
                DesignerEnvironment.CreateDetailLayoutChat = editor => new DetailLayoutChat(AISettings.Instance, editor);
                DesignerEnvironment.CreateCssEditorChat = editor => new CssChat(chatHost, AISettings.Instance, editor);
                DesignerEnvironment.CreateScriptEditorChat = editor => new ScriptChat(chatHost, AISettings.Instance, editor);
                DesignerEnvironment.CreatePageFrameChat = editor => new PageFrameChat(chatHost, AISettings.Instance, editor);
            }
        ...
        private void CreateDDL(SolutionExplorerMenuClickEventArgs e)
        {
            ...
            var ddl = mod.CreateDDL(dataSource.DataSourceType, DesignerEnvironment.GetDbInfo(dataSource.Name));
        ...
        private void CreateFieldClass(SolutionExplorerMenuClickEventArgs e)
        {
            var modName = e.Item.Split(".").First();
            var mod = DesignerEnvironment.GetDesignData().Modules.Find(modName);
            if (mod == null)
            ...
            var classTxt = ClassGenerator.ModuleDesignToFieldClass(mod);
        ...
        {
            using Stream stream = new MemoryStream(AccountingApp.Designer.Properties.Resources.EmptyTemplate);
            ZipFile.ExtractToDirectory(stream, path);
            EnsureSampleDbExtracted("C:\\Codeer.LowCode.Blazor.Local\\Data\\sqlite_sample.db", AccountingApp.Designer.Properties.Resources.sqlite_sample);
        }

        static void CreateEmptyAuth(string path)
        {
            using Stream stream = new MemoryStream(AccountingApp.Designer.Properties.Resources.EmptyAuthTemplate);
            ZipFile.ExtractToDirectory(stream, path);
            EnsureSampleDbExtracted("C:\\Codeer.LowCode.Blazor.Local\\Data\\sqlite_sample_auth.db", AccountingApp.Designer.Properties.Resources.sqlite_sample_auth);
        }

        static void CreateGettingStandard(string path)
        {
            using Stream stream = new MemoryStream(AccountingApp.Designer.Properties.Resources.GettingStartedTemplate);
            ZipFile.ExtractToDirectory(stream, path);
            EnsureSampleDbExtracted("C:\\Codeer.LowCode.Blazor.Local\\Data\\sqlite_sample.db", AccountingApp.Designer.Properties.Resources.sqlite_sample);
        }
        ...
        {
            using Stream stream = new MemoryStream(AccountingApp.Designer.Properties.Resources.InventoryManagementTemplate);
            ZipFile.ExtractToDirectory(stream, path);
            EnsureSampleDbExtracted("C:\\Codeer.LowCode.Blazor.Local\\Data\\inventory_v1.db", AccountingApp.Designer.Properties.Resources.inventory_v1);
        }

        static void CreateSFA(string path)
        {
            using Stream stream = new MemoryStream(AccountingApp.Designer.Properties.Resources.SFATemplate);
            ZipFile.ExtractToDirectory(stream, path);
            EnsureSampleDbExtracted("C:\\Codeer.LowCode.Blazor.Local\\Data\\sfa_v1.db", AccountingApp.Designer.Properties.Resources.sfa_v1);
        }

        static void CreateProjectManagement(string path)
        {
            using Stream stream = new MemoryStream(AccountingApp.Designer.Properties.Resources.ProjectManagementTemplate);
            ZipFile.ExtractToDirectory(stream, path);
            EnsureSampleDbExtracted("C:\\Codeer.LowCode.Blazor.Local\\Data\\project_management_v1.db", AccountingApp.Designer.Properties.Resources.project_management_v1);
        }

        static void CreatePatternShowcase(string path)
        {
            using Stream stream = new MemoryStream(AccountingApp.Designer.Properties.Resources.PatternShowcaseTemplate);
            ZipFile.ExtractToDirectory(stream, path);
            EnsureSampleDbExtracted("C:\\Codeer.LowCode.Blazor.Local\\Data\\sqlite_patterns_v4.db", AccountingApp.Designer.Properties.Resources.sqlite_patterns_v4);
        }

        static void CreatePatternShowcaseAuth(string path)
        {
            using Stream stream = new MemoryStream(AccountingApp.Designer.Properties.Resources.PatternShowcaseAuthTemplate);
            ZipFile.ExtractToDirectory(stream, path);
            EnsureSampleDbExtracted("C:\\Codeer.LowCode.Blazor.Local\\Data\\sqlite_patterns_auth_v3.db", AccountingApp.Designer.Properties.Resources.sqlite_patterns_auth_v3);
        }
