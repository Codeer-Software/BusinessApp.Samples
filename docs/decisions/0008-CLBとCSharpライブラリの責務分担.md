...
updated: 2026-08-27
supersedes: []
related: [0002-部品の定義と分割の判断基準.md, 0004-優良な電子帳簿への準拠と仕訳の不変性.md, 0009-専門家レビューの代替手段.md, 0022-明細を並べる帳簿はクエリモジュールで作る.md, 0025-取引先を部品として分ける.md]
---
# ADR-0008 CLB と C# ライブラリの責務分担

> **2026-08-27 追記**: [ADR-0025](0025-取引先を部品として分ける.md) により取引先が独立部品になり、
> プロジェクト構成に取引先のプロジェクト群（`BusinessApp.Partners` ほか）が加わる。
> `AccountingCore` の「依存: なし」は「`BusinessApp.Partners` のみ（NuGet ゼロは不変）」になる。
> 責務分担の決定（ロジックは C#・CLB は薄いプレゼンテーション層）は変わっていない。
>
