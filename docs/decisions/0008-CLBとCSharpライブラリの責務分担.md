...
updated: 2026-09-18
supersedes: []
related: [../22_層と実行場所.md, 0002-部品の定義と分割の判断基準.md, 0004-優良な電子帳簿への準拠と仕訳の不変性.md, 0009-専門家レビューの代替手段.md, 0022-明細を並べる帳簿はクエリモジュールで作る.md, 0025-取引先を部品として分ける.md]
---
...
> 散っていた規則を集約した）。**本 ADR の規則の記述は、2 つの表も「CLB 側の責務」「CLB から C# を呼ぶ経路」も
> 帰結も、全体が当時の姿である**（現在形は 22 §1-2・§2-3・§3・§4）——
> **決定（ロジックは C#・CLB は薄いプレゼンテーション層）とその理由は変わっていない**ので、
> 全体を `superseded` にしない（[00 §3-3](../00_ドキュメント規約/フロントマター.md)・[qa/02](../qa/02_自己レビュー記録/2026-08.md) の R13-05）。
>
> **2026-08-27 追記**: [ADR-0025](0025-取引先を部品として分ける.md) が取引先を独立部品にすると
> 決めた。実装後、本文のプロジェクト構成の表と `AccountingCore` の「依存: なし」は当時の姿になる
> （新しい構成の正典は ADR-0025 §2）。責務分担の決定（ロジックは C#・CLB は薄い
> プレゼンテーション層）は変わっていない。
>
...
>
> 現在の規約（[00 §3-3](../00_ドキュメント規約/フロントマター.md)）なら、一部でも覆された ADR は全体を `superseded` にして
