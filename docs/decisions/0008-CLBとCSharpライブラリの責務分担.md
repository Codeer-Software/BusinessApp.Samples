...
> **2026-08-26 追記**: 「実行場所の決め方」の表のうち**元帳の行**は
> [ADR-0022](0022-明細を並べる帳簿はクエリモジュールで作る.md) が置き換えた。
> **明細を並べる帳簿（仕訳帳・総勘定元帳・補助元帳）はクエリモジュールで作る。**
> 畳んで初めて意味が出る集計帳票（試算表・B/S・P/L・税集計）がサーバ完結であることは変わらない。
>
> 現在の規約（[00 §3-3](../00_ドキュメント規約.md)）なら、一部でも覆された ADR は全体を `superseded` にして
> 引き継いだ新 ADR を起こす。本 ADR を `current` のまま置いているのは、
> **その規約を決める前に ADR-0022 を書いていたため**である（今回限りの例外。開発者の判断。2026-08-26）。
