...
ADR-0024 で秘密は User Secrets へ出したが、`appsettings.Development.json` には**ローカル絶対パス（`C:\Users\REDACTED\...`）が残ったまま**コミットされていた。ユーザーの指摘: 環境固有情報を Git に入れる理由はもう無く、「環境構築に必要なら、その方法をドキュメントなりサンプルファイルなりに残せばいい」。
