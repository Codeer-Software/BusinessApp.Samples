...
| A-13 | **`MaxLength` は、上限を超えた入力を黙って止める。** 23 文字を打つと 20 文字で止まり、警告もトーストも無い | **`TextFieldDesign` の `MaxLength` は HTML の `maxlength` になり、「拒む」ではなく「止める・切る」**（打鍵は 2026-09-09 実測 1.3.20。マスタのコード。[qa/02 R57-02](02_自己レビュー記録.md)。**貼り付けが 20 文字に切られるのは `maxlength` の仕様からの読みで、実測していない**）。識別子で起きると別のものが生まれる。**使わない**——上限はプレースホルダで見せ、関門が断る（最後に残っていた取引先の法人番号も 2026-09-11 に外し、14 桁で「「法人番号」は数字 13 桁です。いまは 14 文字あります。」と断られることを踏んだ） |
