...
| G-10 | **ログイン画面で `navigate` の直後に入力した内容が消える。** Login を押しても画面が変わらず、入力欄が空に戻っている | **1 回目の入力を捨て、2 回目に入れ直す**（2026-09-02 実測。B・D・E の実機確認で毎回起きた）。`login.html` は素の HTML で、読み込み直後にフォームを初期化するらしい。**「パスワードが違う」と誤診しやすい**——**入力後にスクリーンショットで欄の中身を確かめてから** Login を押す |
