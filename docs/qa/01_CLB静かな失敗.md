...
| A-11 | **空にした文字の欄が、NULL ではなく空文字で保存される。** 見た目は空なのに `is null` で引けない | **`TextFieldDesign` の既定は `TextEditEmptyType: StringEmpty`**（候補は `StringEmpty` / `Null`）。**空白だけを打つとその空白がそのまま入る**（`ShouldTrimAfterEdit` の既定が `false` なので、打った空白が落ちない）。**測ったのは伝票の摘要 1 欄**で、他の欄は未計測（既定が同じなので同じ振る舞いだと見ている）。**空値検索（電帳通達 8-13）が取りこぼす**ので、[10 §4-4](../10_会計ドメイン設計.md) は「無いは NULL で表す」と決めている。**参照欄（`LinkField`）も空文字を送る**（F-33）が、あちらは外部キーが拒むので気づける——**文字の欄は誰も拒まないので黙って入る**。**伝票の摘要だけ `Null` ＋ `ShouldTrimAfterEdit: true` に直した**（**保存先を持つ 35 欄のうち、残り 34 欄は既定のまま**。[04 §1](../04_実装計画と現在地.md) の A-5） |
| A-12 | **`TextEditEmptyType: Null` は「一度入れた値を消したとき」には効くが、`ShouldTrimAfterEdit` が作った空文字は NULL に化けない** | 2026-09-08 実測（1.3.20。伝票の摘要）。**値を入れて保存 → 消して保存**は `description` が **NULL** になる。一方、**空白だけを打つ**と `ShouldTrimAfterEdit` が空文字にするが、それは **`''` のまま保存**される。**2 つの設定は合成されない。** 「無いは NULL」（[10 §4-4](../10_会計ドメイン設計.md)）を本当に守るなら**サーバ側の正規化**が要る（[04 §1](../04_実装計画と現在地.md) の A-5）。**一度も触っていない欄がどちらになるかは未計測**——画面の操作が安定せず、3 回で引いた（[30 §3](../30_作業のルール.md)） |
...
| D-31 | **`Placeholder` は検索欄にも出る。** 詳細画面のつもりで書いた文が、検索の欄に薄字で並ぶ | `Placeholder` は**レイアウト要素ではなくフィールドの属性**なので、同じ欄を検索レイアウトに置くと同じ文が出る（2026-09-08 に伝票の摘要で気づいた）。**「何を書くか」だけを書き、押すボタンや保存のタイミングは書かない** |
...
| F-42 | **関門の差し戻しを例外で返すと、トーストが 2 枚出る**（関門の文言＋「更新に失敗しました」。R25-08）。**関門の外で投げた開発者向けの例外もそのまま画面に出る** | **`ModuleSubmitResult.ExceptionMessage` に文言を載せて返すと、CLB はその 1 枚だけを出し、その保存で書いた分を巻き戻す**（2026-09-10 実測 1.3.20）。手順：下書きの摘要を変えてから「計上する」→ 関門が計上で差し戻す（保存は済んでいる）→ トーストは関門の文言 1 枚、DB の摘要も版も動いていない。**巻き戻しは CLB の挙動に依る**ので、版を上げたら同じ手順で踏み直す。<br>**限界**：**明細だけを直した保存（親の差分が無い。F-41 ②）では、CLB の「更新に失敗しました」が 1 枚残る**（同日実測。結果に行ごとの `SourceId` を付けても同じ）。親の差分がある保存（伝票・マスタ）は 1 枚。[FB-017](../CLB改善提案/01_機能改善提案.md)。差し替えの変換は保存の入口 1 か所（`AccountingSubmitPipeline.SubmitAsResultAsync`）、利用者に見せてよい文言かは型で決める（`RejectedException`。[ADR-0051](../decisions/0051-関門の差し戻しはCLBへ例外ではなく結果で返し利用者に見せる文言は型で決める.md)） |
