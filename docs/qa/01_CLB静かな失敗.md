...
> 2026-08-28 に数えたところ、**96 項目のうち 76 項目が無印**である。1.3.20 で挙動が変わっている
> 可能性があるので、**無印の項目を設計の前提にするときは実機で確かめる。**
...
**次に機械へ移すもの**（[05 のフェーズ 2.5](../05_実装計画と現在地.md)）: **F-15**（`this.Submit()` の前に
`this.ValidateInput()` があるか）・**F-17**（フレーム跨ぎリンクの登録漏れ）・
検索レイアウトの `IsExpanderDefaultOpened`・`IsRequired` とラベルの必須マークの対応。
...
以下は前回プロジェクトで実測（1.3.18 まで）。**権限を入れる前に全部読む。**
...
| F-20 | `AppUser` に読み取り条件を付けたら、マイプロフィール・他画面の `LinkField` 表示が**全部壊れた** | **`AppUser`（`CurrentUser` のソース）に `UserReadCondition` / `UserWriteCondition` の読み取り側を付けない。** 同じことが**他モジュールから `LinkField` / `SelectField` で参照されるマスタ全般**に当たる（勘定科目・部門・取引先・税区分）。**読み取りは全員に開けたまま、`UserWriteCondition` だけ閉じる。** 画面に行かせたくないなら、条件ではなく**フレームと玄関モジュールで到達可否を分ける**（[ADR-0026](../decisions/0026-画面は役割で分け玄関のページフレームを置く.md)） |
| F-21 | サイドバーのリンクを役割で出し分けられない | **`PageFrame` の `Link` は可視条件を持てない。** メニューの表示・非表示は**遷移先モジュールの `UserReadCondition` に完全連動**する。さらに**フレームの `UserReadCondition` はログインユーザーのレコードの列しか参照できない**（関連テーブルを辿れない）ので、**権限は `AppUser` の列で持つしかない**。前回プロジェクトはこの 2 つを CLB への改善提案（FB-009・FB-034）に挙げている |
