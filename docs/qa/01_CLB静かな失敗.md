...
| A-13 | **`MaxLength` は、上限を超えて貼り付けた値を黙って切って保存する。** 25 文字のコードを貼ると 20 文字で保存され、警告もトーストも無い | **`TextFieldDesign` の `MaxLength` は「拒む」ではなく「切る」**（2026-09-08 実測 1.3.20。マスタのコード。[qa/02 R57-02](02_自己レビュー記録.md)）。識別子で起きると別のものが生まれる。**使わない**——上限はプレースホルダで見せ、関門が断る（最後に残っていた取引先の法人番号も 2026-09-11 に外し、14 桁で「法人番号は数字 13 桁です」と断られることを踏んだ） |
...
| D-31 | **`Placeholder` は検索欄には出ない**（1.3.20。2026-09-11 に部門・取引先・振替伝票の一覧で実測——検索欄の `placeholder` 属性はどれも空） | 2026-09-08 に「伝票の摘要の文が検索欄に出た」と記したが、踏み直すと出ていない（何を見て書いたかは残っていない）。**それでも `Placeholder` には「何を書くか」だけを書き、押すボタンや保存のタイミングは書かない**——欄の意味と離れた文は、どこに出ても読み手を迷わせる |
