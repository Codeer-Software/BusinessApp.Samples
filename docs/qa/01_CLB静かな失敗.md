...
**機械へ移す予定のものは [05 のフェーズ 2.5 の D](../05_実装計画と現在地.md) が持つ**（ここには書かない）。
...
> **番号は D-17 の次が D-25 である。** D-18〜D-24 は `tools/clb/lint_design.py` の
> **検査ルールの番号**として先に使ってある（この表に対応する行は無い）。同じ「D-nn」が
> 2 つの意味を持つ状態は望ましくないが、既に検査のメッセージと [05](../05_実装計画と現在地.md) が
> その番号で参照しているので、**ここでは重ねない**ことで衝突を避ける。
