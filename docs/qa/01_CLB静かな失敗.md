...
| A-11 | **空にした文字の欄が、NULL ではなく空文字で保存される。** 見た目は空なのに `is null` で引けない | **`TextFieldDesign` の既定は `TextEditEmptyType: StringEmpty`**（候補は `StringEmpty` / `Null`）。**空白だけを打つとその空白がそのまま入る**（`ShouldTrimAfterEdit` の既定が `false` なので、打った空白が落ちない）。**測ったのは伝票の摘要 1 欄**で、他の欄は未計測（既定が同じなので同じ振る舞いだと見ている）。**空値検索（電帳通達 8-13）が取りこぼす**ので、[20 §7](../20_実装の原則.md) は「無いは NULL で表す」と決めている。**参照欄（`LinkField`）も空文字を送る**（F-33）が、あちらは外部キーが拒むので気づける——**文字の欄は誰も拒まないので黙って入る**。**伝票の摘要だけ `Null` ＋ `ShouldTrimAfterEdit: true` に直した**（サーバ側の正規化は 2026-09-09 に入った——[20 §7](../20_実装の原則.md)）。**2026-09-14 に、上限を置いた 10 欄すべてを `ShouldTrimAfterEdit: true` にした**——落とさないと**画面で数えた字数と断りの「いまは N 文字」が合わない**（[12 §2-2](../12_マスタ台帳.md)）。**`TextEditEmptyType` を `Null` にしたのは明細の「内容」だけ**で、**名前の欄は `NOT NULL` なので `StringEmpty` のまま**である（`Null` にすると、空にした瞬間に定型文が出る） |
| A-12 | **`TextEditEmptyType: Null` は「一度入れた値を消したとき」には効くが、`ShouldTrimAfterEdit` が作った空文字は NULL に化けない** | 2026-09-08 実測（1.3.20。伝票の摘要）。**値を入れて保存 → 消して保存**は `description` が **NULL** になる。一方、**空白だけを打つ**と `ShouldTrimAfterEdit` が空文字にするが、それは **`''` のまま保存**される。**2 つの設定は合成されない。** 「無いは NULL」（[20 §7](../20_実装の原則.md)）を本当に守るなら**サーバ側の正規化**が要る（2026-09-09 に入った。[20 §7](../20_実装の原則.md)）。**一度も触っていない欄がどちらになるかは未計測**——画面の操作が安定せず、3 回で引いた（[30 §3](../30_作業のルール.md)） |
