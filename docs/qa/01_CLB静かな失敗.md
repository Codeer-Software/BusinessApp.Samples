...
| A-12 | **`TextEditEmptyType: Null` は「一度入れた値を消したとき」には効くが、`ShouldTrimAfterEdit` が作った空文字は NULL に化けない** | 2026-09-08 実測（1.3.20。伝票の摘要）。**値を入れて保存 → 消して保存**は `description` が **NULL** になる。一方、**空白だけを打つ**と `ShouldTrimAfterEdit` が空文字にするが、それは **`''` のまま保存**される。**2 つの設定は合成されない。** 「無いは NULL」（[20 §7](../20_実装の原則.md)）を本当に守るなら**サーバ側の正規化**が要る（2026-09-09 に入った。[20 §7](../20_実装の原則.md)）。**一度も触っていない欄がどちらになるかは未計測**——画面の操作が安定せず、3 回で引いた（[30 §3](../30_作業のルール.md)）。**検索欄の話は E-09 が別に持つ**（`SearchValue` は `ShouldTrimAfterEdit` を通らない） |
