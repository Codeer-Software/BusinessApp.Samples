...
| D-12 | トーストの文言に改行を入れても**表示に出ない**。違反を複数並べると 1 行に繋がって読めない | HTML では改行が空白に潰れる。**LF でも CRLF でも同じ**（2026-08-26 に開発者が実機で確認）。**利用者に見せる文言に改行を入れない**——入れても効かないうえ、改行コードの管理だけが増える。複数の項目を並べるなら 1 行で読める区切りにする |
