...
**F-15（`Submit()` の前の `ValidateInput()`）**・**F-17（遷移先での登録漏れ）**・**F-18（空の読み書き条件）**・**F-32（必須の印の二重）**・**F-34（レイアウトが取ってこない欄の読み）**・**F-06（行の条件が見る欄をレイアウトが取ってこない）**・**F-43（初期値の欄と離脱の確認）**・**D-17（ヘッダ＋明細の親 FK）**
＋ 論理削除の不在（本プロジェクトの方針）。**それ以外は人が見る。**
**検査のルール番号（D-13 と D-17〜D-30・D-32〜D-34）はここの症状の番号とは別の体系である**（下の D 節の注記）。
...
> **検査ルールの側は D-31 を飛ばしてある**——本書の症状 D-31（プレースホルダ）が同じ「検索欄」の話で、引くときに必ず迷うため（2026-09-16 に実物を数えた。いまの検査ルールは D-13 と D-17〜D-30・D-32〜D-34）。
