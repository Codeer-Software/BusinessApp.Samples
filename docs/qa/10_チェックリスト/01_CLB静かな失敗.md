...
### CLB-006 **スクリプト定義関数**の `ref` / `out` 引数

- **概要**: CLB スクリプトでは、**スクリプト内で定義した関数**の `ref`/`out` の書き戻しが呼び出し元に伝わらない。
- **症状**: コンパイルエラーにも実行時エラーにもならず、**値だけが落ちる**（消費税額の合計が常に 0 になった実例）。
- **⚠️ .NET 組み込みメソッドは対象外（2026-08-17 訂正）**: `int.TryParse(s, out x)` /
  `decimal.TryParse` / `DateTime.TryParse` / `Dictionary.TryGetValue` などの **.NET 組み込みメソッドの
  `out`/`ref` は正常に動く**。`Designer/ClaudeCodeForDesigner/_specs/Scripts.md` の
  「out / ref パラメータ」節が `int.TryParse("100", out x)` を**サポート構文として明記**している。
  FB-039 で実測した対象は**ユーザー定義スクリプト関数** `int PostOne(Draft d, ref int tax)` であり、
  組み込みメソッドではない。実際 `JournalLineDepartment` は `int.TryParse(raw, out id)` のまま
  2026-08-14 に実機検証を通っている（`docs/05_実装計画と進捗台帳.md:815-817`）。
  **この区別をせずに `[(,]\s*(?:ref|out)\s+\w` だけで検出すると、本リポジトリでは 4 件すべてが誤検知になる。**
- **検査対象**: `*.mod.cs`
- **検出方法**: 正規表現 `[(,]\s*(?:ref|out)\s+\w` で候補を取り、**その `ref`/`out` を含む引数リストの直前の
  呼び出し名**を取り出す（直前の `(` まで括弧の深さを数えて戻り、その手前の識別子を読む）。
  全 `*.mod.cs` から**スクリプト定義関数の名前集合**を作り（列 0 から `戻り型 名前(` で始まる行）、
  呼び出し名がその集合に含まれ、かつ**レシーバ付きでない**（直前が `.` でない）ときだけ違反とする。
  この条件は「呼び出し側」と「定義シグネチャ側」の両方に同じ形で当たる。
- **違反の条件**: 呼び出し名がスクリプト定義関数のとき。
- **誤検知の可能性**: 文字列リテラルやコメント内の "ref"（コメント・文字列を空白化してから走らせる）。
  スクリプト定義関数と同名の .NET メソッドを呼んでいる場合（現状なし）。
- **重大度**: 高
- **出典**: FB-039 / ADR-0055 / Project.md 2026-08-14 / `_specs/Scripts.md`「out / ref パラメータ」
...
- **⚠️ 検出条件を絞ること（2026-08-17 訂正）**: 次の 2 つを**除外しないと本リポジトリでは全件が誤検知**になる。
  ① `AnchorTagFieldDesign`（`BackAnchor` 等の戻るリンク）。仕様書の初版は AnchorTag も数えていたが、
  実害の報告が無く、4 件中 1 件（`BankStatementLine`）は戻るリンクだけで占められていた。
  ② **可視性をスクリプトで制御しているボタン**。`X.IsVisible = false` の literal だけでなく、
  `SubmitButton.IsVisible = !locked;` `ConfirmButton.IsVisible = ... && (st == "draft");` のように
  **式で隠している**ものが実際には多数派で、これらはロック時には描画されない＝「見た目は有効・実は無反応」に
  ならない。`X.IsVisible` への代入があれば対象から外す。
  この 2 つを除外すると本リポジトリの検出は 4 件 → **0 件**になり、CLB-004 と同じ「新規混入の検出器」として働く。
...
- **⚠️ 検出条件を絞ること（2026-08-17 訂正）**: FB-035 の実測条件は
  「`DbTable: ""` **かつ CanCreate/CanUpdate/CanDelete が全て false**」である。
  `DbTable` 空だけを条件にすると、`CanUpdate: true` の表示専用モジュール
  （`CashEntry` / `BankImport` / `BankPosting` / `JournalImport` / `JournalEntryBoard` /
  `JournalLineDepartment` / `BudgetEntry` / `FbExport` / `RecurringRun` / `SesBilling` の 10 件）を
  全部拾ってしまうが、これらは**解除が無いまま実機で動いている**。とくに `JournalLineDepartment` は
  ADR-0056 の検証項目（伝票 No.89 の部門変更→保存）を通っており、ボタンが死んでいない証拠がある。
  `CanCreate || CanUpdate || CanDelete` が真のモジュールは対象外にすること（検出 10 件 → 0 件）。
...
- **誤検知の可能性**: `new` しているが `Submit()` しない（読み取り用のインスタンス化）ケース。
  `new` から同一メソッド末尾までに `Submit(` が無ければ**報告しない**（2026-08-17 訂正。初版は warn に落とす
  としていたが、`FbExport` の `new PartnerBank(...)` が唯一の該当例で、実体は読み取り用インスタンス化だった。
  「作るだけで保存しない」のは違反ではないので出さない。検出 1 件 → 0 件）。
- **重大度**: 中
...
- **誤検知の可能性**: `this.Delete()`（自モジュールの削除・UI 経路）は成功する前提で書けるため
  **報告しない**（2026-08-17 訂正。初版は warn 止まりとしていたが、13 件中 8 件が `this.Delete()` で
  ノイズの主成分になっていた。検索インスタンスの `Delete()` だけを見れば 5 件に絞れる）。
- **重大度**: 中
...
### CLB-037 `DataWriteCondition` が参照する列が、その画面で読み込まれていない

- **概要**: 書き込み条件は**画面側で評価される**。条件が見る自モジュールの列が、
  レイアウトにも `DataOnlyFields` にも無いと、その値は遅延ロードで null のままになる。
- **症状**: 条件が常に偽になり、**そのレコードの全項目が読み取り専用に描画される**。
  一覧の行削除ボタンも消える。**新規レコードだけは編集できる**（行条件は新規追加に効かないため）ので、
  「追加はできるのに直せない・消せない」という形で現れる。エラーもトーストも出ない。
- **なぜ読み取り条件では起きないか**: `DataReadCondition` は SQL の WHERE で効くため、
  画面にその列がロードされていなくても正しく絞られる。**書き込み条件だけがこの罠を持つ**。
- **機械検査**: 各モジュールの `DataWriteCondition`（`ModuleName` が空＝自モジュール基準のもの）が
  参照する `SearchTargetVariable` の先頭要素を取り出し、`DetailLayouts` / `ListLayouts` の
  レイアウト内 `FieldName` と `DataOnlyFields` の和集合に含まれるかを見る。含まれなければ error。
- **出典**: 2026-08-18 実測（BUG-0323）。経費明細に行条件を入れた翌日から丸 1 日、
  申請者が自分の明細を直せない状態が続いていた。静的監査 208 所見にも不変条件 42 本にも出なかった。

### CLB-038 資金繰り SQL の複製（ポータル ⇄ 予測画面）が食い違っている

- **概要**: プロジェクト固有の規則。`Shell/PortalAlertData.Query.sql` と
  `Management/CashFlowForecastData.Query.sql` は共通 CTE を**逐語コピー**で共有している（90 行規模）。
- **症状**: 片方だけ直すと、ポータルの「今後 4 ヶ月中 N ヶ月」と画面の警告行数が**黙ってずれる**。
  どちらも数字を出し続けるので、見ている人には食い違いが分からない。
- **機械検査**: 共通 CTE（`cash_now` / `inv_in` / `rec_in` / `ap_now` / `exp_now` / `vend_out` / `sal_out`）の
  本文を、コメントと空白を正規化したうえで文字列比較する。一致しなければ error。
- **本来の直し方**: ビュー化して 1 か所に畳む（`docs/tasks/04` の SQL 刷新と同時）。
  **この検査はそれまでの保険**であり、ビュー化したら不要になる。
- **出典**: BUG-0257 ／ ADR-0060（判定の二重実装は必ず食い違う）。2026-08-18 に新設。
