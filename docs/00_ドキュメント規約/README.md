...
| error | **節への参照の指し先に、その節が実在しない**（「リンクの札に節を書く形」・`docs/04 §1`・`docs/09_画面の原則.md §2`・`09 §3`・`CLAUDE.md §5` の 5 形。**リンク切れ検査はファイルの実在しか見ない**ので、節が消えた・番号が動いたときはここだけが鳴る。限界は [§4-12-1](本文.md)） |
