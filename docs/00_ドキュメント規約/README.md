...
| error | **節への参照の指し先に、その節が実在しない**（「リンクの札に節を書く形」・`docs/10 §1`・`docs/21_画面の原則.md §2`・`21 §3`・`CLAUDE.md §5` の 5 形。**リンク切れ検査はファイルの実在しか見ない**ので、節が消えた・番号が動いたときはここだけが鳴る。限界は [§4-12-1](本文.md)） |
...
| 帯 | 意味 | いま入っているもの |
|---|---|---|
| **0x** | **全体** — 文書そのものの約束と、プロジェクトの輪郭と現在地 | 00 本規約・01 企画概要・02 ペルソナ・03 機能スコープ・04 実装計画と現在地 |
| **1x** | **設計** — 何がどうあるべきか（What） | 10 会計ドメイン・11 消費税・12 マスタ台帳・13 取引先 |
| **2x** | **原則** — 成果物が満たすこと | 20 実装・21 画面・22 調査 |
| **3x** | **ルール** — Claude がどう動くか | 30 作業・31 検証 |
| **4x** | **適合** — 外部の要求と、自分たちの成果物の対応づけ | 40 優良な電子帳簿の対応表 |
| **8x** | **参照** — 通読せず、必要なときに 1 本だけ開くもの | 80 参照法令一覧・81 略語と記号 |
| **9x** | **その他** — どの帯にも入らないもの | （空。受け皿として空けてある） |

**番号を付けるのは「読む文書」だけである。** `decisions/`・`research/`・`qa/`・
`CLB改善提案/` は 1 件 1 ファイルで**たまっていくもの**なので番号を持たない。
**例外は本規約（`00_ドキュメント規約/`）だけ**——1 つの規約を 3 冊に割ったものであり、
中身は読む文書だからである。

### 8-1. 番号の付け方

- **足す**: その帯の**最大 +1**。帯の中は連番で、空けない（**成長の余地は帯そのものが持つ**）
- **帯を増やさない。** 増やしたくなったら、それは分類が間違っている合図である
- **「その他」（9x）が 2 本を超えたら、分類を見直す**——受け皿は逃げ場であって置き場ではない
- **文書の役割が変わったら番号も動く。** これがこの体系の代金である
  （連番なら引っ越しは要らない）。**動かすときは [§4-12-1](本文.md) の手順で参照を洗う**——
  旧番号は別の実在文書に着地するので、**§6 の検査は鳴らない**
