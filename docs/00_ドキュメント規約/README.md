...
| error | **条項を 80 §3 の記法で書いていない**（`5 条 1 項`・`第 5 条第 1 項`。**法令名に錨を下ろして拾う**ので「12 項目」「4 条件」「令和 8 年法律第 12 号」は当たらない。`.md` とコードの両方を見る。**書いてよい 4 つは [80 §3-1](../80_参照法令一覧.md) が持つ**——外す行は `lint-docs:ignore`。開発者の指示 2026-09-06） |
