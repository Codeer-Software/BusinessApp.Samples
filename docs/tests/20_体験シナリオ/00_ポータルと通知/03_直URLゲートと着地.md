...
> **フレームは 15 本ある。** 業務フレーム 14 本のほかに **`/Settings`（パスワード変更）**があり、
> こちらは `UserReadCondition` が空＝**全ログインユーザーが入れる**（誰でも自分のパスワードは変えられる必要があるため）。
> 門番を試して回る対象からは外れるが、存在は知っておく。
