...
>
> **注記（2026-08-09）**: 本票は**当時の環境の記録**（CLB 1.2.51・旧フレーム体制・admin が業務画面に入れた時代）。現行は CLB 1.3.16／14フレーム体制で、admin は業務画面に入れない（ADR-0041/0043）。
>
...
> 現行体制での読み替え: 手順1は経理ユーザー（soumu_bucho 等）でログインし、手順4は `/Accounting` →「設定/会計年度」を開く（admin は業務画面に入れない）。
